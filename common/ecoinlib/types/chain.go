...
func InitChain(args InitChainArgs) (c *Chain, err error) {
	...
	err = db.Update(func(txn *badger.Txn) error {
		...
		log.Success("Genesis created: %s", genesisBlock.Hash)
...
func (c *Chain) MineBlock(txs []TX, gsm *GlobalStateMachine) (b *Block, err error) {
	...
	newBlock = NewBlock(txs, BytesToHash(lastHash), lastId+1, gsm.opts.UserID())
...
func (c *Chain) GetBlockById(id int) (b *Block, err error) {
	...
	if id > maxId || id < (-maxId-1) {
		return nil, utils.WrapError("Chain_GetBlockById", ErrOutOfChainRange)
	}
...
func (c *Chain) GetBlockHashes() (blockHashes []Hash, err error) {
	...
	for {
		...
		if block.PrevHash == Hash([32]byte{}) {
			break
		}
	}
...
func (c *Chain) PrintBlockHeaders(start, end int) error {
	...
	if maxId < end {
		return utils.WrapError("Chain_PrintBlockHeaders", ErrOutOfChainRange)
	}
	...
	for {
		...
		if currentBlock.Id == end+1 || currentBlock.PrevHash == Hash([32]byte{}) {
			break
		}
	}
...
func (c *Chain) FindTransaction(txId Hash) (tx TX, err error) {
	...
	for {
		...
		if block.PrevHash == Hash([32]byte{}) {		// TODO： 理解： 这里为什么 不能直接用Hash{} 因为编译器不认识，会将其认为是判断后的执行程序块
			break
		}
	}
