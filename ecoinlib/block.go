...
func (b *Block) String() string {
	...
	bytes1, err := JsonMarshalIndent(blockForPrint)
	if err != nil {
		return err.Error()
	}
...
func GenesisBlock(coinbase *TxCoinbase) (gb *Block, err error) {
	...
	if err != nil {
		return nil, WrapError("GenesisBlock", err)
	}
...
func (b *Block) Deserialize(blockBytes []byte) (err error) {
	...
	if err = decoder.Decode(b); err != nil {
		return WrapError("Block_Deserialize", err)
	}
	return nil
}
...
func (b *Block) PrintTransactions() {
	...
	for i, txBytes := range b.Transactions {
		tx, err = DeserializeTX(b.TxTypes[i], txBytes)
		if err != nil {
...
func (b *Block) VerifyTXs(gsm *GlobalStateMachine) (txTypeNoArray [][]uint, err error) {
	...
	// 使用TX.IsValid检查所有交易
	for i, txBytes := range b.Transactions {
		if tx, err = DeserializeTX(b.TxTypes[i], txBytes); err != nil {
			return nil, err
		}
		...
		case TX_COINBASE:	// TX_COINBASE = 1
			if b.CreateBy != tx.(*TxCoinbase).To {
...
func (b *Block) IsValid(gsm *GlobalStateMachine) (err error) {
	...
	//}
	merkle := NewMerkleTree(b.Transactions)
	if string(merkle.RootNode.Data) != string(b.Hash) {
		return WrapError("Block_IsValid", ErrInconsistentMerkleRoot)
	}
