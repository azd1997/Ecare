...
func (a *AddrLists) SaveFileWithJsonMarshal(port uint) (err error) {
	filePath := fmt.Sprintf(ADDRS_FILEPATH_TEMP, strconv.Itoa(int(port)))

	if err = saveFileWithJsonMarshal(filePath, a); err != nil {
		return WrapError("AddrList_SaveFile", err)
	}
	return nil
}

// LoadFileWithJsonUnmarshal 从本地还原出AddrList。 addrs := &AddrList{}
func (a *AddrLists) LoadFileWithJsonUnmarshal(port uint) (err error) {

	file := fmt.Sprintf(ADDRS_FILEPATH_TEMP, strconv.Itoa(int(port)))
	if _, err = os.Stat(file); os.IsNotExist(err) {
		return WrapError("AddrLists_LoadFile", err)
	}
