...
import (
	...
	"github.com/azd1997/Ecare/ecoin/p2p"
...
func NewP2PTCPNode(args *p2p.Args) *TCPNode {
	return &TCPNode{
		Server:etcp.NewServer(
			etcp.DefaultOption().SetHost(args.Ip).SetPort(args.Port).SetName(args.Name)),
...
func (n *TCPNode) Start(args p2p.Args) {
...
func (n *TCPNode) Push(remote *p2p.Address, key string, value []byte, version []byte)  error {
...
// Pull 从远程拉取数据
func (n *TCPNode) Pull(remote p2p.Address, key string, version []byte) error {
